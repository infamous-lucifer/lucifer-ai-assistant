...
import { isPathAllowed, resolveFilePath, isDangerousCommand, applySearchAndReplace, showVisualDiff, highlightMarkdown, truncateOutput, Spinner, pruneHistory, getLogsToDelete } from './utils.js';
// --- Dynamic Path Resolution ---
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
// If running from 'dist', the project root is one level up
const PROJECT_ROOT = process.env.LUCIFER_HOME || (__dirname.endsWith('dist') ? path.join(__dirname, '..') : __dirname);
...
let manifest = { version: "9.2", dependencies: [], dangerPatterns: [], tools: [] };
try {
...
async function runSetupWizard() {
    ...
    if (key.trim())
        fs.writeFileSync(CONFIG_FILE, `API_KEY=${key.trim()}\n`);
    const lmsOk = await rl.question('Is LM Studio installed? (y/n): ');
    if (lmsOk.toLowerCase() !== 'y')
        console.log(chalk.yellow('⚠ Please install it from lmstudio.ai\n'));
...
if (args.includes('--last')) {
    ...
    if (logs.length > 0)
        execFileSync('open', [path.join(LOGS_DIR, logs[0])]);
    else
        ...
        switch (name) {
            ...
            case "search_web": {
                ...
                try {
                    ...
                    return truncateOutput(results.map((r) => `[${r.title}](${r.url})\n${r.abstract}`).join('\n\n'), 1500);
                }
                catch (e) {
            ...
            case "search_and_replace": {
                ...
                try {
                    ...
                    if (edPath.includes("index.ts"))
                        fs.copyFileSync(edPath, BACKUP_FILE);
            ...
            case "propose_fix": {
                ...
                if (!isPathAllowed(reviewPath, ALLOWED_ROOTS))
    ...
    while (true) {
        try {
            ...
            if (query.startsWith('!fix')) {
                ...
                if (!issue) {
                    console.log(chalk.yellow("Usage: !fix <issue description>"));
                    continue;
                }
            ...
            if (query.startsWith('!search')) {
                ...
                if (!searchQuery) {
                    console.log(chalk.yellow("Usage: !search <your query>"));
                    continue;
                }
            ...
            if (query.startsWith('!read')) {
                ...
                if (!readPath) {
                    console.log(chalk.yellow("Usage: !read <file path>"));
                    continue;
                }
                try {
            ...
            if (query.startsWith('!tldr')) {
                ...
                if (!cmdName) {
                    console.log(chalk.yellow("Usage: !tldr <command>"));
                    continue;
                }
            ...
            while (loopCount < 5) {
                    ...
                    if (delta?.tool_calls) {
                        for (const toolCallDelta of delta.tool_calls) {
                            ...
                            if (!toolCalls[toolCallDelta.index])
                                toolCalls[toolCallDelta.index] = { id: toolCallDelta.id, type: "function", function: { name: "", arguments: "" } };
                            if (toolCallDelta.id)
                ...
                console.log('\n');
                const assistantMsg = { role: 'assistant', content: assistantMsgContent };
                if (toolCalls.length > 0)
                    assistantMsg.tool_calls = toolCalls;
                history.push(assistantMsg);
                if (assistantMsgContent)
                    finalResponse = assistantMsgContent;
            ...
            if (loopCount > 1) {
                try {
                    execFileSync('osascript', ['-e', 'display notification "Task complete" with title "Lucifer"']);
                }
                catch { }
            }
        }
